...
build_services() {
    ...
    # 병렬 빌드
    log "Backend, Frontend, MCP Proxy 병렬 빌드 중..."
...
restart_services() {
    ...
    # 프론트엔드 및 기타 서비스 시작
    log "프론트엔드 및 기타 서비스 시작 중..."
    docker compose -f "$COMPOSE_FILE" --env-file "$ENV_FILE" up -d frontend mcp-proxy victoriametrics grafana redis-exporter
...
health_checks() {
    ...
    # 순서대로 헬스 체크
    check_health "postgres" 30
    check_health "redis" 20
    check_health "pgbouncer" 20
    check_health "backend" 60
    check_health "frontend" 40
    check_health "mcp-proxy" 30
    check_health "victoriametrics" 30
    check_health "grafana" 30
