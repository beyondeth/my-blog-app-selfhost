...
정의: 어떤 데이터나 로직은 단 하나의 출처(source)에서만 관리되고, 나머지는 그걸 참조해서 사용하는 원칙.
목적:
중복 제거 → 같은 데이터가 여러 군데 흩어져 있으면 불일치 발생 가능
유지보수 용이 → 한 곳만 수정하면 전체가 반영됨
신뢰성 확보 → "이 데이터는 여기에서만 진짜다" 라는 기준이 생김

코드 작성 원칙 (Clean Code / OOP / FP 등에서 자주 쓰는 것)
SRP (Single Responsibility Principle, 단일 책임 원칙)
클래스, 모듈, 함수는 하나의 책임만 가져야 한다.
👉 "이 코드가 바뀌는 이유는 단 하나여야 한다."
DRY (Don't Repeat Yourself, 중복 배제 원칙)
같은 로직을 여러 번 작성하지 말자.
👉 "한 번만 정의하고, 여러 군데서 재사용해라."
KISS (Keep It Simple, Stupid)
설계와 코드를 최대한 단순하게 유지해라.
👉 "괜히 복잡하게 만들지 말고, 이해하기 쉽게."
YAGNI (You Aren't Gonna Need It)
필요하지 않은 기능은 미리 만들지 말자.
👉 "미래를 대비한 쓸데없는 코드 작성 금지."
SSOT (Single Source of Truth, 단일 진실 공급원)
어떤 데이터/로직은 단 한 곳에서만 관리해야 한다.
👉 "여러 군데 흩어져 있으면 오류의 원인이 된다."