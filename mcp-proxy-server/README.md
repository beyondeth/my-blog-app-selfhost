...
- **Separate Instance**: Independent deployment (mcp.aigory.com)
...
```json
{
  "mcpServers": {
    "aigory_blog": {
      "url": "https://mcp.aigory.com/api/v1/mcp",
      "transport": "http"
    }
  }
}
...
docker run -d \
  ...
  -e BACKEND_BASE_URL=https://api.aigory.com \
  -e REDIS_HOST=redis \
  --name mcp-proxy \
  mcp-proxy-server
```
