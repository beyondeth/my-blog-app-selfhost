...
async function readSkillDocument(fileName) {
  ...
  throw new Error(
    `Missing aigory-blog document ${fileName} in skills/aigory-blog or .agents/skills`,
  );
}
...
async function verifyStaticParity() {
  ...
  for (const { name, requiredTools } of docsFiles) {
    ...
    if (missing.length > 0) {
      fail(
        `aigory-blog/${name} is missing tool names: ${missing.join(", ")}`,
      );
    } else {
      ok(`aigory-blog/${name} includes required tool names`);
    }
  }
