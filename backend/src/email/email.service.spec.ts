...
describe("EmailService verification safeguards", () => {
  ...
  const createVerification = (
    ...
    ({
      ...
      code: null,
      codeHash: (service as any).hashVerificationCode(
        "person@example.com",
        "123456",
      ),
      isVerified: false,
  ...
  it("rejects an expired verification code without consuming an attempt", async () => {
    ...
    await expect(
      service.verifyCode(verification.email, "123456"),
  ...
  it("returns a session token and marks the verification as verified for the correct code", async () => {
    ...
    const result = await service.verifyCode(verification.email, "123456");
  ...
  it("does not emit the full verification code to logs during SMTP delivery", async () => {
    ...
    try {
      ...
      expect(verification.code ?? null).toBeNull();
      expect(verification.codeHash).toMatch(/^[a-f0-9]{64}$/);
