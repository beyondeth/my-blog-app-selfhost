...
export class OptionalJwtAuthGuard extends AuthGuard('jwt') {
  ...
  canActivate(context: ExecutionContext) {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      this.logger.debug(`Cookies: ${request.cookies ? Object.keys(request.cookies).join(', ') : 'No cookies'}`);
    }
  ...
  handleRequest(err: any, user: any, info: any, context: ExecutionContext) {
    ...
    // 민감정보 (user.id) 제거
    if (process.env.NODE_ENV === 'development') {
      this.logger.debug(`Authentication result: error=${err ? err.message : 'none'}, user=${user ? 'authenticated' : 'null'}, info=${info ? info.message : 'none'}`);
    }
