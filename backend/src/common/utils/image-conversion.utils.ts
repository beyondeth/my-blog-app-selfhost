 ...
 * 컨텍스트별 필요한 썸네일 사이즈
 */
export function getRequiredThumbnailSizes(
  context: string,
...
export function getOptimizationConfig(
  ...
  const baseConfig =
    CONTEXT_CONVERSION_RULES[context] || DEFAULT_CONVERSION_CONFIG;
...
export function getConversionLogMessage(
  ...
  return (
    `Image conversion: ${fileName} | ${originalFormat} → ${targetFormat} | ` +
