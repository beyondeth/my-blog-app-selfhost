...
export class SubscriptionBasicController {
  ...
  async getPaymentHistory(@Request() req) {
    const history = await this.subscriptionService.getPaymentHistory(req.user.id);
    return {
      success: true,
      data: history,
    };
  ...
  async createCheckoutSession(
    @Request() req,
    @Body() body: { tier: string; billingCycle: 'monthly' | 'yearly'; provider?: string }
  ) {
  ...
  async cancelSubscription(
    ...
    return {
      success: true,
      data: subscription,
      message: '구독이 취소되었습니다. 현재 결제 주기가 끝날 때까지 서비스를 이용할 수 있습니다.',
    };
  }
