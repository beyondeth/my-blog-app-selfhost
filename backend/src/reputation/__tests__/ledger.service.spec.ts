...
describe('LedgerService', () => {
  ...
  describe('record', () => {
    ...
    it('POST_PUBLISHED 액션에 기본 점수가 적용되어야 함', async () => {
      ...
      const expectedDelta = REPUTATION_ACTION_SCORES[ReputationAction.POST_PUBLISHED];
  ...
  describe('REPUTATION_ACTION_SCORES (exported constant)', () => {
    it('각 액션에 올바른 점수가 매핑되어야 함', () => {
      ...
      expect(REPUTATION_ACTION_SCORES[ReputationAction.POST_PUBLISHED]).toBe(10);
      ...
      expect(REPUTATION_ACTION_SCORES[ReputationAction.BOOKMARK_RECEIVED]).toBe(1);
    });
  });
});
