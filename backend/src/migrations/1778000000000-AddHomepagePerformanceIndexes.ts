...
export class AddHomepagePerformanceIndexes1778000000000
  implements MigrationInterface
{
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    try {
      // 생성된 인덱스 롤백
      await queryRunner.query(
        `DROP INDEX IF EXISTS "idx_posts_homepage_covering";`,
      );
      await queryRunner.query(
        `DROP INDEX IF EXISTS "idx_posts_cursor_optimized";`,
      );
      await queryRunner.query(
        `DROP INDEX IF EXISTS "idx_posts_author_published_composite";`,
      );
      await queryRunner.query(
        `DROP INDEX IF EXISTS "idx_posts_blog_published_composite";`,
      );
