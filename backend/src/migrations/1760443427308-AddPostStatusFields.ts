...
export class AddPostStatusFields1760443427308 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX IF EXISTS "public"."idx_comments_post"`);
        await queryRunner.query(`DROP INDEX IF EXISTS "public"."idx_posts_search_published"`);
        await queryRunner.query(`DROP INDEX IF EXISTS "public"."idx_posts_blog"`);
        await queryRunner.query(`DROP INDEX IF EXISTS "public"."idx_posts_published_date"`);
        await queryRunner.query(`DROP INDEX IF EXISTS "public"."idx_posts_search_vector"`);
        await queryRunner.query(`DROP INDEX IF EXISTS "public"."idx_posts_author"`);
        await queryRunner.query(`DROP INDEX IF EXISTS "public"."IDX_POSTS_EDITOR_PICK"`);

        // 필드가 이미 존재할 수 있으므로 체크
        await queryRunner.query(`ALTER TABLE "posts" ADD COLUMN IF NOT EXISTS "status" character varying NOT NULL DEFAULT 'published'`);
        await queryRunner.query(`ALTER TABLE "posts" ADD COLUMN IF NOT EXISTS "processing_error" text`);
        await queryRunner.query(`ALTER TABLE "posts" ADD COLUMN IF NOT EXISTS "processing_completed_at" TIMESTAMP`);
        ...
        if (!enumCheck[0].has_old_enum) {
            ...
            const hasPartiallyRefunded = currentEnum.some((row: any) => row.enumlabel === 'partially_refunded');
        ...
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "IDX_a69d9e2ae78ef7d100f8317ae0" ON "posts" ("status")`);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "IDX_c63b5acddc521623c55af299ac" ON "posts" ("isEditorPick", "editorPickedAt")`);
    }
