...
export class AddUserSubscriptionFields1759091000000
  implements MigrationInterface
{
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    // 컬럼 삭제
    await queryRunner.query(
      `ALTER TABLE "users" DROP COLUMN IF EXISTS "paymentCustomerId"`,
    );
    await queryRunner.query(
      `ALTER TABLE "users" DROP COLUMN IF EXISTS "trialEndDate"`,
    );
    await queryRunner.query(
      `ALTER TABLE "users" DROP COLUMN IF EXISTS "subscriptionEndDate"`,
    );
    await queryRunner.query(
      `ALTER TABLE "users" DROP COLUMN IF EXISTS "subscriptionStartDate"`,
    );
    await queryRunner.query(
    ...
    // enum 타입 삭제
    await queryRunner.query(
      `DROP TYPE IF EXISTS "users_subscriptionstatus_enum"`,
    );
    await queryRunner.query(
      `DROP TYPE IF EXISTS "users_subscriptiontier_enum"`,
    );
  }
}
