...
export class OptimizeCacheWarmingQueries1759200000000
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    try {
      ...
      console.log(
        "⚠️ 통계 업데이트 실패 (수동 실행 필요): ANALYZE posts, blogs, users",
      );
    }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    // 생성한 인덱스들 제거
    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_posts_published_created_desc"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_posts_published_publishedat_desc"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_posts_blog_published_publishedat"`,
    );
    ...
    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_posts_author_publishedat"`,
    );
