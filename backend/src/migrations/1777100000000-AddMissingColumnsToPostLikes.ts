...
export class AddMissingColumnsToPostLikes1777100000000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        try {
            ...
            for (const constraint of constraints) {
                try {
                    await queryRunner.query(`ALTER TABLE post_likes DROP CONSTRAINT "${constraint.conname}"`);
                } catch (error) {
            ...
            try {
                ...
                console.log('Enum type already exists or creation failed:', error.message);
            }
            ...
            console.log(`Successfully migrated ${existingData.length} existing post_likes records`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        try {
            ...
            // 제약조건 제거
            try {
                await queryRunner.query(`ALTER TABLE post_likes DROP CONSTRAINT IF EXISTS "UQ_post_likes_user_post_type"`);
                await queryRunner.query(`ALTER TABLE post_likes DROP CONSTRAINT IF EXISTS "PK_post_likes"`);
            } catch (error) {
                console.log('Could not drop constraints:', error.message);
            }

            // 컬럼 제거
            await queryRunner.query(`ALTER TABLE post_likes DROP COLUMN IF EXISTS "createdAt"`);
            await queryRunner.query(`ALTER TABLE post_likes DROP COLUMN IF EXISTS type`);
            await queryRunner.query(`ALTER TABLE post_likes DROP COLUMN IF EXISTS id`);
