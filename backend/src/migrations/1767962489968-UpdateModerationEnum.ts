...
export class UpdateModerationEnum1767962489968 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        if (!(await checkValue('SUSPEND_USER'))) {
             await queryRunner.query(`ALTER TYPE "public"."moderation_action_enum" ADD VALUE 'SUSPEND_USER'`);
        }
        if (!(await checkValue('UNBAN_USER'))) {
             await queryRunner.query(`ALTER TYPE "public"."moderation_action_enum" ADD VALUE 'UNBAN_USER'`);
        }
        if (!(await checkValue('BLOCK_IP'))) {
             await queryRunner.query(`ALTER TYPE "public"."moderation_action_enum" ADD VALUE 'BLOCK_IP'`);
        }
        if (!(await checkValue('UNBLOCK_IP'))) {
             await queryRunner.query(`ALTER TYPE "public"."moderation_action_enum" ADD VALUE 'UNBLOCK_IP'`);
        }
    }
