...
export class CacheInvalidationListener {
  ...
  async handlePostChange(payload: { postId: string; blogSlug?: string }) {
    this.logger.debug(`🔄 [Post Change] Invalidating cache for: ${payload.postId}`);
  ...
  async handleUserProfileUpdated(payload: { userId: string }) {
    ...
    try {
      ...
      this.logger.warn(`Failed to fetch user blog for cache invalidation: ${error.message}`);
    }
  }
  ...
  private async invalidatePatterns(patterns: string[]): Promise<void> {
    try {
      ...
      if (failed.length > 0) {
        this.logger.warn(`⚠️ Cache invalidation failed for ${failed.length} patterns`);
      }
