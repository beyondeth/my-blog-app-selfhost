...
describe("Cache Invalidation Events (Unit)", () => {
  ...
  it("CacheInvalidationEvents enum - 모든 이벤트 타입 검증", () => {
    ...
    expectedEvents.forEach((eventType) => {
      const exists = Object.values(CacheInvalidationEvents).includes(
        eventType as any,
      );
  ...
  it("COMMENT_CREATED 이벤트 - 댓글 페이지 + 포스트 캐시 무효화", () => {
    ...
    // Assert: 검증
    expect(invalidationPatterns).toContain(
      `comments:page:first:${payload.postId}:*`,
    );
    expect(invalidationPatterns).toContain(
  ...
  it("USER_PROFILE_UPDATED 이벤트 - 프로필 이미지 변경 시 피드 무효화", () => {
    ...
    console.log(
      "✅ USER_PROFILE_UPDATED 무효화 패턴 검증 성공:",
      invalidationPatterns,
    );
  });
