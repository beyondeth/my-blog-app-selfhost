...
export class ConversationRepository {
  ...
  async markUserAsLeft(conversationId: string, userId: string): Promise<void> {
    ...
    const result = await this.repository
      ...
      .update(Conversation)
      .set({
        user1DeletedAt: () => `CASE WHEN user1Id = :userId THEN NOW() ELSE user1DeletedAt END`,
        user2DeletedAt: () => `CASE WHEN user2Id = :userId THEN NOW() ELSE user2DeletedAt END`,
      })
    ...
    if (result.affected === 0) {
      console.warn(`[대화] 대화 ${conversationId}에서 사용자 ${userId}를 찾을 수 없음`);
    }
  }

  async resetUserDeletedAt(conversationId: string, userId: string): Promise<void> {
    // 조건부 UPDATE로 최적화 - DB 조회 없이 한 번의 쿼리로 처리
    ...
    const result = await this.repository
      ...
      .update(Conversation)
      .set({
        user1DeletedAt: () => `CASE WHEN user1Id = :userId THEN NULL ELSE user1DeletedAt END`,
        user2DeletedAt: () => `CASE WHEN user2Id = :userId THEN NULL ELSE user2DeletedAt END`,
      })
      .where('id = :conversationId', { conversationId })
      .andWhere('(user1Id = :userId OR user2Id = :userId)', { userId })
      // deletedAt이 설정된 경우에만 리셋 (불필요한 UPDATE 방지)
      .andWhere('(user1Id = :userId AND user1DeletedAt IS NOT NULL) OR (user2Id = :userId AND user2DeletedAt IS NOT NULL)', { userId })
      .execute();

    if (result.affected === 0) {
      console.log(`[대화] 대화 ${conversationId}에서 사용자 ${userId}의 deletedAt이 이미 null이거나 참여자가 아님`);
    }
  }
}