  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleDisconnect(client: Socket) {
    try {
      ...
      if (userId) {
        ...
        await this.unifiedRedisService.deleteCache('chat', `socket:${client.id}`);
  ...
  async handleMessage(
    ...
    try {
      ...
      const recipientSocketId = await this.unifiedRedisService.getCache<string>('chat', `online:${recipientId}`);
  ...
  async handleTyping(
    ...
    if (data.isTyping) {
      ...
      await this.unifiedRedisService.deleteCache('chat', `typing:${data.conversationId}:${userId}`);
    }
