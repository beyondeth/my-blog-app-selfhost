...
@Index(["isDeleted"])
@Index(["status"])
@Index(["blogId", "likeCount"]) // 인기순 정렬 최적화
@Index(["blogId", "viewCount"]) // 조회순 정렬 최적화
@Index(["blogId", "createdAt"]) // 최신순 정렬 최적화 (이미 id v7로 어느정도 되지만 명시적)
export class Post {
  /**
   * 기본 키
   * - UUID v7 (시간 순서 정렬 지원)
   ...
   *   - SEO는 메타데이터(title, description)로 처리
   */
  @BeforeInsert()
  generateUuidAndSlug() {
    // UUID 생성
