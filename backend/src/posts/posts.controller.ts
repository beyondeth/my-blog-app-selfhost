...
export class PostsController {
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: "블로그 alias (@alias 형식)",
  })
  ...
  async findAll(
    ...
    if (blogSlug && !blogId) {
      try {
        // blogSlug에서 @ 제거
        const cleanAlias = blogSlug.startsWith("@")
          ? blogSlug.slice(1)
          : blogSlug;
        const blog =
          await this.blogResolverService.resolveBlogByIdentifier(cleanAlias);
        actualBlogId = blog?.id;
      } catch (error) {
        this.logger.warn(
          `Failed to resolve blogSlug ${blogSlug} to blogId:`,
          error,
        );
        actualBlogId = null;
      }
    ...
    if (limit && parseInt(limit, 10) > 20) {
      ...
      this.logger.warn(
        `Suspicious limit request: IP=${req.ip}, limit=${attemptedLimit}, user=${user?.id || "anonymous"}`,
      );

      // 모니터링 서비스에 기록
      this.monitoringService
        .logExcessiveLimitRequest(
          req.ip || "unknown",
          "/api/v1/posts",
          attemptedLimit,
          limitNumber,
  ...
  @ApiOperation({
    ...
    description:
      "Fast Path로 생성된 포스트의 백그라운드 처리 상태를 확인합니다. status 값: processing(처리중), published(완료), failed(실패)",
  })
  ...
  @Roles(Role.ADMIN)
  @ApiBearerAuth()
  @ApiOperation({ summary: "Editor's Pick 목록 조회 (관리자)" })
  @ApiQuery({
    name: "limit",
    required: false,
    type: Number,
    description: "조회할 개수 (기본: 10, 최대: 10)",
  })
  @ApiResponse({ status: 200, description: "Editor's Pick 목록 반환 (관리자)" })
  async getEditorPicksAdmin(@Query("limit") limit?: string) {
    const limitNumber = limit ? parseInt(limit, 10) : 10;
    const safeLimit = Math.min(limitNumber, 10);
    const posts = await this.postsService.findEditorPicksAdmin(safeLimit);
  ...
  @Patch("editor-picks/order")
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(Role.ADMIN)
  @ApiBearerAuth()
  @ApiOperation({ summary: "Editor's Pick 순서 변경 (관리자)" })
  async updateEditorPicksOrder(
    @Body() dto: UpdateEditorPicksOrderDto,
    @CurrentUser() user: User,
  ) {
  ...
  async update(
    ...
    if ("thumbnailImageId" in updatePostDto || "thumbnail" in updatePostDto) {
      this.logger.log(
        "🎯 [THUMBNAIL_TRACK] STEP_4_BACKEND_RECEIVE: Post update request received",
      );
      this.logger.debug(`  - Post ID: ${id}`);
      this.logger.debug(`  - User ID: ${user.id}`);
      this.logger.debug(
        `  - thumbnailImageId: ${updatePostDto.thumbnailImageId}`,
      );
      this.logger.debug(
        `  - thumbnail: (removed - using thumbnailImageId only)`,
      );
    ...
    if ("thumbnailImageId" in updatePostDto || "thumbnail" in updatePostDto) {
      this.logger.log(
        "🎯 [THUMBNAIL_TRACK] STEP_4_BACKEND_SUCCESS: Post update completed",
      );
    }
  ...
  @Public()
  @UseGuards(OptionalJwtAuthGuard)
  @ApiOperation({ summary: "게시글 투표 (upvote/downvote)" })
  ...
  async vote(
    ...
    const result = await this.voteService.toggleVote(id, user.id, dto.type);
   ...
   * 좋아요 토글 (레거시)
   *
   * @deprecated POST /:id/vote 사용 권장
   */
  @Post(":id/like")
  @Public()
  @UseGuards(OptionalJwtAuthGuard)
  @ApiOperation({
    summary: "[Deprecated] 게시글 좋아요 토글",
    description: "POST /:id/vote 사용 권장. 내부적으로 upvote로 처리됩니다.",
    deprecated: true,
  })
  async toggleLike(
    @Param("id", ParseUUIDPipe) id: string,
    ...
    // 레거시 API는 VoteService의 toggleVote로 위임 (upvote로 처리)
  ...
  @ApiOperation({
    ...
    description:
      "무한 스크롤을 위한 커서 기반 페이지네이션. 대규모 데이터셋에서도 일정한 성능 보장 (O(1))",
  })
  @ApiQuery({
    name: "cursor",
    required: false,
    type: String,
    description: "Base64 인코딩된 커서 (첫 페이지는 생략)",
  })
  @ApiQuery({
    name: "limit",
    required: false,
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: "블로그 슬러그 필터",
  })
  @ApiQuery({
    name: "blogId",
    required: false,
    type: String,
    description: "블로그 ID 필터",
  })
  @ApiQuery({
    name: "search",
    required: false,
