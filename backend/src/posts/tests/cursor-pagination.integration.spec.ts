...
describe("Cursor Pagination (Integration)", () => {
  ...
  it("[GET /posts/cursor] 첫 페이지 조회 - cursor 없이 최신 포스트 20개 반환", async () => {
    ...
    const response = {
      ...
      nextCursor: Buffer.from(`2025-01-20T12:00:00.000Z|post-19`).toString(
        "base64",
      ),
      hasMore: true,
      count: 20,
  ...
  it("[GET /posts/cursor?cursor=XXX] nextCursor로 다음 페이지 조회", async () => {
    ...
    const firstPageCursor = Buffer.from(
      `2025-01-20T12:00:00.000Z|post-19`,
    ).toString("base64");
    ...
    const response = {
      ...
      nextCursor: Buffer.from(`2025-01-20T11:50:00.000Z|post-39`).toString(
        "base64",
      ),
      hasMore: true,
      count: 20,
  ...
  it("[GET /posts/cursor] 마지막 페이지 hasMore=false 검증", async () => {
    ...
    const lastPageCursor = Buffer.from(
      `2025-01-20T10:00:00.000Z|post-95`,
    ).toString("base64");
  ...
  it("[GET /posts/cursor?sort=popular] 인기순 정렬 검증", async () => {
    ...
    console.log(
      "✅ 인기순 정렬 검증 성공:",
      sorted.map((p) => ({ id: p.id, score: p.score })),
    );
  });
  ...
  it("[GET /posts/cursor?sort=trending] 트렌딩 정렬 검증 (최근 7일)", async () => {
    ...
    console.log(
      "✅ 트렌딩 정렬 검증 성공:",
      trending.map((p) => ({ id: p.id, score: p.score })),
    );
  });
