...
export class IdentityService {
  ...
  async linkIdentity(
    ...
    const existingIdentity = await this.findByProviderId(
      data.providerId,
      data.provider,
    );
    ...
    const hasProvider = userIdentities.some(
      (i) => i.provider === data.provider,
    );
    ...
    this.logger.log(
      `Successfully linked ${data.provider} identity to user ${userId}`,
    );
  ...
  async unlinkIdentity(userId: string, identityId: string): Promise<void> {
    ...
    this.logger.log(
      `Unlinked ${identityToRemove.provider} identity from user ${userId}`,
    );
  }
  ...
  async setPrimaryIdentity(userId: string, identityId: string): Promise<void> {
    ...
    this.logger.log(
      `Set primary identity to ${identity.provider} for user ${userId}`,
    );
  }
  ...
  isTrustedProvider(provider: IdentityProvider): boolean {
    return [IdentityProvider.GOOGLE, IdentityProvider.GITHUB].includes(
      provider,
    );
  }
  ...
  canAutoLink(provider: IdentityProvider, email: string, user: User): boolean {
    ...
    if (user.isEmailVerified) {
      return [IdentityProvider.KAKAO, IdentityProvider.NAVER].includes(
        provider,
      );
    }
  ...
  async reclaimAccount(
    ...
    this.logger.log(
      `Reclaiming unverified account ${email} through ${provider} OAuth`,
    );
    ...
    this.logger.log(
      `Successfully reclaimed account ${email} through ${provider}`,
    );
    return identity;
  }
}
