...
export class UserDeletionDebugService {
   ...
   * 삭제 전 사용자 데이터 수집
   */
  async collectPreDeletionData(
    userIdOrEmail: string,
  ): Promise<DeletionDebugInfo> {
    this.logger.log(
      `🔍 [DEBUG] Collecting pre-deletion data for user: ${userIdOrEmail}`,
    );
    ...
    try {
      ...
      this.logger.log(
        `🔍 [DEBUG] Input type detection - isUuid: ${isUuid}, value: ${userIdOrEmail}`,
      );
      ...
      if (isUuid) {
        ...
        this.logger.log(
          `🔍 [DEBUG] Raw query result: ${JSON.stringify(rawUser?.length)} users found`,
        );
        ...
        if (rawUser && rawUser.length > 0) {
          ...
          this.logger.log(
            `🔍 [DEBUG] User data from raw query: ${JSON.stringify(userData)}`,
          );
      ...
      this.logger.log(
        `🔍 [DEBUG] User found: ${user ? `Yes (ID: ${user.id}, Email: ${user.email})` : "No"}`,
      );
      ...
      const reports = await queryRunner.manager
        ...
        .where(
          'report."reportedById" = :userId OR (report.type = :userType AND report."targetId" = :userId)',
      ...
      // 총 데이터 크기 계산
      const totalDataSize = files.reduce(
      ...
      this.logger.log(
        `  - Files: ${files.length} (Total size: ${this.formatBytes(totalDataSize)})`,
      );
  ...
  async verifyDeletion(userId: string): Promise<void> {
    ...
    try {
      ...
      this.debugInfo.afterDeletion.verificationStatus =
        orphanedData.length === 0 ? "clean" : "orphaned_data_found";
