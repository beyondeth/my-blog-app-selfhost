...
import type { BlogsService } from "../blogs/blogs.service";
...
export function createAdminUserInput(
  email: string,
  username: string,
  password: string,
) {
...
export function validateExistingAdmin(user: any, email: string): void {
  if (user.isDeleted) {
    throw new Error(
      `A deleted user already exists for ${email}. Use account recovery instead of admin:create.`,
    );
  }

  if (user.role !== Role.ADMIN) {
    throw new Error(
      `A user already exists for ${email}. Use admin:promote to grant the admin role without creating a new account.`,
    );
  }

  if (!user.isEmailVerified || !user.isActive) {
    throw new Error(
      `The existing admin account for ${email} is inactive or unverified. Resolve that account state before using admin:create.`,
    );
  }
}
...
async function main(): Promise<void> {
  ...
  const [{ AppModule }, { UsersService }, { BlogsService }] = await Promise.all(
    [
      import("../app.module"),
      import("../users/users.service"),
      import("../blogs/blogs.service"),
    ],
  );
  ...
  try {
    ...
    if (user) {
      validateExistingAdmin(user, email);
    } else {
      if (!password) {
        throw new Error(
          "Usage: pnpm admin:create -- --email admin@example.com --password 'StrongPassword123!' [--username admin]",
      ...
      user = await usersService.create(
        createAdminUserInput(email, username, password) as any,
      );
    }
