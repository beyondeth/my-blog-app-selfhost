...
export class CommentsService {
  ...
  async create(
    ...
    const savedComment = await this.commentsRepository.manager.transaction(
      async (manager) => {
        const transactionalCommentsRepository = manager.getRepository(Comment);
        const saved = (await transactionalCommentsRepository.save(
          comment,
        )) as unknown as Comment;

        await this.outboxService.enqueue(manager, {
          eventType: PostInteractionEvents.COMMENT_ADDED,
