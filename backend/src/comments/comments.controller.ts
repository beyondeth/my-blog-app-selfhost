...
export class CommentsController {
  ...
  @ApiQuery({ name: 'cursor', required: false, description: 'Base64 인코딩된 커서' })
  @ApiQuery({ name: 'limit', required: false, description: '페이지당 댓글 개수 (1-50)', type: Number })
  @ApiQuery({ name: 'sort', required: false, enum: ['recent', 'popular'], description: '정렬 방식' })
  @ApiQuery({ name: 'snapshotTimestamp', required: false, description: '스냅샷 타임스탬프 (인기순 시 필수)' })
  ...
  @ApiQuery({ name: 'cursor', required: false, description: 'Base64 인코딩된 커서' })
  @ApiQuery({ name: 'limit', required: false, description: '페이지당 답글 개수 (1-50)', type: Number })
  getReplies(
