...
import { SubscriptionTier, BillingCycle, ResourceType } from '../common/enums/subscription.enum';
...
export class SubscriptionFacadeService {
  ...
  async createCheckoutSession(data: {
    ...
    // 현재 구독 상태 확인
    const currentSubscription = await this.subscriptionService.getUserSubscription(data.userId);
    ...
    // 다운그레이드 체크
    if (data.tier === SubscriptionTier.FREE || data.tier === SubscriptionTier.STARTER) {
      const canDowngrade = await this.subscriptionService.canDowngrade(data.userId, data.tier);
      if (!canDowngrade) {
        throw new Error(
  ...
  async simulateUpgrade(
    ...
    const selectedPrice = billingCycle === BillingCycle.MONTHLY ? monthlyPrice : yearlyPrice;
