...
export class FileMonitoringService {
  ...
  async healthCheck(): Promise<HealthCheckResult> {
    ...
    const s3Status: "operational" | "unavailable" = "operational";
  ...
  async cleanupRecommendations(): Promise<string[]> {
    ...
    if (orphanedCount > 0) {
      recommendations.push(
        `Run orphaned file cleanup (${orphanedCount} files)`,
      );
    }
    ...
    if (v1Count > 0) {
      recommendations.push(
        `Migrate v1 files to v2 structure (${v1Count} files)`,
      );
    }
    ...
    if (expiredCount > 0) {
      recommendations.push(
        `Process expired files for deletion (${expiredCount} files)`,
      );
    }
  ...
  async cleanupExpiredFiles(): Promise<void> {
    ...
    this.logger.log(
      `Cleanup completed. Processed ${expiredFiles.length} files.`,
    );
  }
