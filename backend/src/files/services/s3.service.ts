...
export class S3Service {
  ...
  /**
   * Read a small prefix of an object for content-signature validation without
   * downloading the complete upload.
   */
  async getObjectSample(fileKey: string, bytes = 64): Promise<Buffer | null> {
    try {
      const response = await this.s3Client.send(
        new GetObjectCommand({
          Bucket: this.bucket,
          Key: fileKey,
          Range: `bytes=0-${Math.max(1, bytes) - 1}`,
        }),
      );
      ...
      this.logger.warn(`Failed to read object sample: ${error.message}`);
      return null;
    }
  }
