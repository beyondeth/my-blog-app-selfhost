...
describe('FileMonitoringService', () => {
  ...
  describe('healthCheck', () => {
    it('should return healthy status when all systems are operational', async () => {
      // Arrange
      const files = [
        MockFactory.createMockFile({ fileKey: 'v2/users/user-1/post/content/file1.jpg' }),
        MockFactory.createMockFile({ fileKey: 'v2/users/user-2/profile/avatar/file2.jpg' }),
      ];
      const contexts = [
    ...
    it('should return unhealthy when database is unavailable', async () => {
      // Arrange
      fileRepository.count.mockRejectedValueOnce(new Error('Database connection failed'));
  ...
  describe('getStorageUsageByUser', () => {
    ...
    it('should sort users by total size descending', async () => {
      ...
      const files = [
        ...
        MockFactory.createMockFile({ userId: 'user-large', fileSize: 1024 * 1000 }),
        MockFactory.createMockFile({ userId: 'user-medium', fileSize: 1024 * 100 }),
      ];
