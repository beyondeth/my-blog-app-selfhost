...
describe('FileMigrationService', () => {
  ...
  beforeEach(async () => {
    ...
    contextualFileService = {
      createContext: jest.fn().mockImplementation((type, id, purpose, ownerId) => {
      ...
      findOrCreateContext: jest.fn().mockImplementation((type, id, purpose, ownerId) => {
        const existing = contextRepository.getData().find(c => 
          c.contextType === type && c.contextId === id && c.purpose === purpose
        );
        if (existing) return Promise.resolve(existing);
        return contextualFileService.createContext(type, id, purpose, ownerId);
      }),
    };
  ...
  describe('analyzeExistingFiles', () => {
    it('should correctly identify v1 and v2 files', async () => {
      // Arrange
      const v1Files = [
        MockFactory.createMockFile({ fileKey: 'uploads/images/2024/01/file1.jpg' }),
        MockFactory.createMockFile({ fileKey: 'uploads/documents/2024/01/file2.pdf' }),
      ];
      const v2Files = [
        MockFactory.createMockFile({ fileKey: 'v2/users/user-123/post/content/file3.jpg' }),
        MockFactory.createMockFile({ fileKey: 'v2/users/user-456/profile/avatar/file4.png' }),
      ];
  ...
  describe('migrateToV2', () => {
    it('should migrate v1 file to v2 structure', async () => {
      ...
      expect(s3Service.deleteFile).toHaveBeenCalledWith('uploads/images/2024/01/test.jpg');
  ...
  describe('migrateFileToContext', () => {
    it('should detect and migrate post attachment', async () => {
      ...
      expect(migratedFile!.fileKey).toMatch(/^v2\/users\/user-123\/post\/content\//);
    });
    ...
    it('should detect profile image', async () => {
      ...
      expect(migratedFile!.fileKey).toMatch(/^v2\/users\/user-123\/profile\/avatar\//);
    });
  ...
  describe('rollback', () => {
    it('should rollback migration on critical failure', async () => {
      ...
      // First file succeeds
      s3Service.copyFile.mockResolvedValueOnce({ success: true, sourceKey: '', destKey: '' });
      // Second file fails
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle concurrent migrations safely', async () => {
      ...
      const totalProcessed = results.reduce((sum, r) => sum + r.progress.processed, 0);
