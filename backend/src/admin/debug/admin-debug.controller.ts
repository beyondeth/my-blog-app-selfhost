...
export class AdminDebugController {
  ...
  async previewDeletion(@Param('userIdOrEmail') userIdOrEmail: string) {
    try {
      ...
      const debugInfo = await this.debugService.collectPreDeletionData(userIdOrEmail);
      ...
      throw new NotFoundException(`사용자를 찾을 수 없습니다: ${userIdOrEmail} - ${error.message}`);
    }
  }
  ...
  async simulateDeletion(@Param('userIdOrEmail') userIdOrEmail: string) {
    ...
    return {
      ...
      throw new NotFoundException(`사용자를 찾을 수 없습니다: ${userIdOrEmail}`);
    }
  }
  ...
  async executeRealDeletion(
    ...
    try {
      // 삭제 전 데이터 수집
      const preData = await this.debugService.collectPreDeletionData(userIdOrEmail);
      ...
      return {
        ...
        summary: {
          ...
          deletedData: {
            ...
            totalFileSize: this.formatBytes(preData.beforeDeletion.totalDataSize),
          },
