...
import { FiShield, FiFileText, FiUsers, FiMail } from 'react-icons/fi';
...
const faqs = [
  {
    ...
    answer:
      '설정 > API 키에서 블로그 범위의 키를 만든 뒤 직접 MCP, MCPorter OAuth, Secure MCP Tunnel 중 한 경로를 선택합니다. 발행 전에는 항상 check_auth의 인증 방식을 확인하세요.',
    hasLink: true,
  },
  {
    question: 'MCP 사용은 무료인가요?',
    answer:
      '셀프호스트 버전은 별도 유료 MCP 요금제를 포함하지 않습니다. 서버 운영비, AI 제공자 비용, 저장소 비용과 배포 환경의 rate limit은 각 운영자가 관리합니다.',
  },
  {
    question: '계정을 삭제하려면 어떻게 해야 하나요?',
    answer:
      "프로필 설정 메뉴에서 '계정 삭제'를 진행할 수 있습니다. 모든 데이터가 영구적으로 삭제되며 복구가 불가합니다. 재가입은 30일 이후 가능합니다.",
  },
  {
  ...
  },
  {
    ...
    answer:
      '프로필 이미지는 파일당 5MB, 게시글 이미지는 파일당 10MB까지 업로드할 수 있습니다.',
  },
  {
    question: '신고된 콘텐츠는 어떻게 처리되나요?',
    answer:
      '신고된 콘텐츠는 관리자가 검토하여 커뮤니티 가이드라인 위반 여부를 판단합니다. 위반이 확인되면 콘텐츠 삭제, 경고, 계정 정지 등의 조치가 취해집니다.',
  },
];
    ...
    <div className="min-h-screen bg-background dark:bg-[#0E141B]">
      ...
      <div className="mx-auto max-w-6xl px-4 py-12 lg:ml-32">
        ...
        <section className="rounded-lg border border-gray-200 dark:border-gray-700 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-700 p-8 text-center">
          ...
          <a
            href="mailto:support@aigory.com"
