...
const STYLE_NAMES: Record<string, string> = {
  ...
  research: 'Research - 근거 중심 논문 분석',
  human: 'Human - 경험 중심의 성찰적 글쓰기',
...
export default function WritingStyleDetailPage() {
  ...
  if (loading) {
    return (
      <div
        className={`${SETTINGS_PAGE_WRAPPER} flex items-center justify-center py-20`}
      >
  ...
  if (error) {
    return (
      ...
      >
        <div
          className={`${SETTINGS_CARD_CLASS} max-w-md w-full p-6 text-center space-y-4`}
        >
        ...
        <section className={`${SETTINGS_CARD_CLASS} p-6 sm:p-8 space-y-4`}>
          <div className="flex flex-col gap-3">
            ...
            <p className="text-gray-600 dark:text-gray-400">
              MCP 자동포스팅에서{' '}
              <code className="px-1.5 py-0.5 bg-gray-100 dark:bg-[#1F2229] rounded text-sm font-mono">
                --{style}
              </code>{' '}
              플래그를 사용할 때 참고하세요.
            </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              onClick={handleCopy}
              className={`${SETTINGS_PRIMARY_BUTTON_CLASS} flex items-center gap-2`}
            >
              {copied ? (
                <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              ...
              {frontmatter.code_block_ratio && (
                <div>
                  ...
                  <p className="font-medium text-gray-900 dark:text-gray-100">
                    {(parseFloat(frontmatter.code_block_ratio) * 100).toFixed(
                      0,
                    )}
                    %
                  </p>
        ...
        <section className={`${SETTINGS_CARD_CLASS} p-6 sm:p-8 space-y-4`}>
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              아래 내용을 복사해 로컬{' '}
              <code className="px-1.5 py-0.5 bg-gray-100 dark:bg-[#1F2229] rounded font-mono text-xs">
                .md
              </code>{' '}
              파일로 저장한 뒤 자유롭게 수정하세요.
            </p>
          </div>
