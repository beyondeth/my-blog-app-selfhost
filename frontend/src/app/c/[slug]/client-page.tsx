...
import { useCommunityPosts, useCommunityPostVote, useDeleteCommunityPost, useTogglePostPin } from '@/hooks/community/useCommunityPosts';
...
import type { Community, CommunityPost, CommunityPostSortByType } from '@/types/community';
 ...
 * - 사이드바 (커뮤니티 정보, 규칙)
 */
export default function CommunityDetailClient({ initialCommunity, slug }: CommunityDetailClientProps) {
  ...
  } = useCommunity(slug, { initialData: initialCommunity });
  ...
  // 게시물 고정 훅
  const { mutate: togglePin } = useTogglePostPin(slug);

  const handlePin = useCallback((postId: string, postSlug: string, isPinned: boolean) => {
    togglePin({ postId, postSlug, isPinned });
  }, [togglePin]);
      ...
      <div className="min-h-screen w-full bg-white text-[#1B2430] dark:bg-[#0E141B] dark:text-[#E6EDF3]">
        <div className="max-w-7xl mx-auto px-6 pb-16 pt-16">
          ...
          >
            ...
            {/* 공지사항 (NoticeBoard) */}
            <NoticeBoard notices={pinnedPosts.map(post => adaptCommunityPostToFeedPost(post))} communitySlug={slug} />
            ...
            {isPostsLoading ? (
              ...
              <div className="max-w-[780px] mx-auto space-y-3">
                {regularPosts.map((post, index) => {
                  ...
                  // 모더레이터 권한 확인
                  // 모더레이터 권한 확인
                  const isModerator = (community.userMembership?.role && isModeratorOrAbove(community.userMembership.role)) || isAdmin;
                  ...
                  return (
                    ...
                    >
                      <PostArticle
                        ...
                        onVote={handleVote}
                        onPin={isModerator ? handlePin : undefined}
