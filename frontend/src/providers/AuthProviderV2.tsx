...
export function AuthProviderV2({ children }: { children: React.ReactNode }) {
  ...
  const isConsentComplete = Boolean(
    authValue.user?.termsAcceptedAt && authValue.user?.privacyAcceptedAt
  );
  ...
  // OAuth 로그인 후 /admin에서 시작한 로그인 흐름을 복원한다.
  // 약관 동의가 필요한 신규 사용자는 consent 페이지를 먼저 완료한다.
  useEffect(() => {
    if (
  ...
  }, [authValue.isAuthenticated, isConsentComplete]);
