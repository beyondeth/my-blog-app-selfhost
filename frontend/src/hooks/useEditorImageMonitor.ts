...
export function useEditorImageMonitor({
  ...
  /**
   * Extract all image IDs from the editor
   */
  const extractImageIds = useCallback((editor: Editor): Set<string> => {
    ...
    console.log('[EditorMonitor] 📊 Total images in editor:', imageIds.size, Array.from(imageIds));
    return imageIds;
  }, []);
  ...
  useEffect(() => {
    ...
    const handleTransaction = ({ transaction }: any) => {
      ...
      if (transaction.docChanged) {
        ...
        // Check if the change involves images
        let hasImageChange = false;
        transaction.steps.forEach((step: any) => {
          const stepJSON = step.toJSON();
          if (stepJSON && JSON.stringify(stepJSON).includes('image')) {
            hasImageChange = true;
          }
        });
        
        if (hasImageChange) {
          console.log('[EditorMonitor] 🖼️ Image-related transaction detected!');
        }
